...
#undef HDEBUG
#define HDEBUG(fmt, ...) do { printf("DEBUG:"fmt"\n", ##__VA_ARGS__); }while(0)

#undef HERROR
#define HERROR(fmt, ...) do { printf("ERROR:"fmt"\n", ##__VA_ARGS__); }while(0)

#undef HINFO
#define HINFO(fmt, ...) do { printf("INFO:"fmt"\n", ##__VA_ARGS__); }while(0)
...
int main(int argc, char* argv[]) {
    ...
    HINFO("hash count=%d", hash_count(h));
    ...
    /* get known value */
    {
        ...
        while (buf[i]) {
            ...
            HINFO("key=%s, value=%s", buf[i],  value ? value:"");
            i++;
        }
    }
    ...
    /* get unknown value */
    {
        ...
        HINFO("key=%s, value=%s", key,  value ? value:"");
